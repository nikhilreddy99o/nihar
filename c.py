...
def create_branch_repo():
    display_location()
    read_choice = input("Do you want to create a new branch or repository? (branch/repo) ")
    if read_choice.lower() == "branch":
        new_branch_name = input("Enter the name of the new branch: ")
        subprocess.run(['git', 'checkout', '-b', new_branch_name])
    elif read_choice.lower() == "repo":
        new_repo_name = input("Enter the name of the new repository: ")
        subprocess.run(['git', 'init', new_repo_name])
    else:
        print("Invalid choice. Exiting...")
        exit(1)

    select_choice = input("Do you want to select the newly created branch/repository? (y/n) ")
    if select_choice.lower() == "y":
        list_branches_repos()
